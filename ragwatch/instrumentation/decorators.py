...
def _resolve_adapter(name: Optional[str]):
    ...
    if name is None:
        return None
    from ragwatch.adapters.base import get_adapter
    ...
    *,
    ...
    if callable(func):
        return _make_wrapper(func, span_name=None, span_kind=span_kind,
                             ...
                             span_hooks=span_hooks, adapter=adapter)
...
def _make_wrapper(
    ...
    *,
    ...
    _adapter = _resolve_adapter(adapter)
    ...
    if asyncio.iscoroutinefunction(func):
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            with tracer.start_as_current_span(resolved_name) as span:
                ...
                state = _resolve_state(_adapter, args, kwargs)
                ctx = InstrumentationContext(
                    span=span, span_name=resolved_name,
                    span_kind=span_kind, func_name=func.__qualname__,
                    args=args, kwargs=kwargs, adapter=_adapter,
                    state=state,
                )
...
def _safe_transform(
    ctx: InstrumentationContext, raw: Any, result_formatter: Any,
) -> Any:
    """Call transform_result with failure isolation."""
    try:
        return transform_result(
            ctx.span, ctx.span_kind, ctx.args, ctx.kwargs,
            raw, result_formatter, context=ctx,
        )
    except Exception as exc:
...
def _extract_node_telemetry(
    ...
    get_default_registry().extract_all(
        telemetry, ctx.span, ctx.span_name, ctx.args, ctx.raw_result,
        kwargs=ctx.kwargs, adapter=ctx.adapter, context=ctx,
    )
